...
class I18nNavigationFrontend {
  ...
  private static function getMenuHTML(&$menu, $showTitles=false, $componentname=null) {
    ...
    return self::getMenuHTML2($menu, $showTitles, $component);
  }

  public static function getMenuHTML2(&$menu, $showTitles=false, $component=null) {
    $html = '';
    foreach ($menu as &$item) {
      ...
      if (isset($component)) {
        ...
        if (isset($item['children']) && count($item['children']) > 0) {
          $html .= '<ul>' . self::getMenuHTML2($item['children'], $showTitles, $component) . '</ul>';
        }
        $html .= '</li>' . "\n";
      }
    }
...
class I18nNavigationItem {
  ...
  public function outputChildren() {
    if (isset($this->item['children']) && count($this->item['children']) > 0) {
      echo I18nNavigationFrontend::getMenuHTML2($this->item['children'], $this->showTitles, $this->component);
    }
  }
